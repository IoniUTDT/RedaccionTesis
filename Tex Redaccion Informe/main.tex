...
\subsection{Calibraciones y mediciones preliminares.} \label{resultados:preliminares}
	...
	\begin{figure}
        ...
        \caption{Comparación entre los umbrales detectados por los sujetos según las diferentes orientaciones y categorías a las que pertenecían. En cada caso se comparó sólo muestras en donde los sujetos que habían realizado ambas pruebas superando en cada una test que distinguiera mediciones de respuestas aleatorias.}
        \label{fig:Exp1_ResultadosDetalle}
    \end{figure}  
    
    En la figura \ref{fig:Exp1_ResultadosDetalle} se puede observar la comparación final entre todos los datos estadísticamente comparables. EVIDENTEMENTE HAY UNA CUENTA MAL EN EL ULTIMO que no encoentre!, pero mas allá de eso se puede observar que en los experimentos de ángulos, es menos marcado el efecto de la proximidad de los estímulos a los ejes. Esta diferencia se puede explicar en el hecho de que aunque los segmentos que componen las figuras estén cercanos a los ejes, cada uno de ellos esta cercano a ejes diferentes, por lo que el efecto de posible comparación entre ambos descripto en la sección \ref{resultados:preliminares} no es aplicable, sino que los segmentos deberían ser comparados contra un estímulo imaginado.
